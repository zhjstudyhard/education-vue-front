...
import { addComment, delComment,addVideoComment } from "../api/article/comment";
...
export default {
  ...
  methods: {
    ...
    addComment() {
      ...
      addVideoComment(this.commentForm).then((response) => {
        this.$message({
          type: "success",
          message: "评论成功!",
          offset: 130,
  ...
  created() {
    if (this.$route.params.id) {
      this.commentForm.courseId = this.$route.params.id;
    }
  },
};
</script>
