<template>
  <div id="aCoursesList" class="bg-fa of">
    <Navbar class="navbar" id="nav"></Navbar>
    <!-- 讲师介绍 开始 -->
    <section class="container">
      <header class="comm-title">
        <h2 class="fl tac">
          <span class="c-333">讲师介绍</span>
        </h2>
      </header>
      <div class="t-infor-wrap">
        <!-- 讲师基本信息 -->
        <section class="fl t-infor-box c-desc-content">
          <div class="mt20 ml20">
            <section class="t-infor-pic">
              <img :src="teacher.avatar" />
            </section>
            <h3 class="hLh30">
              <span class="fsize24 c-333"
                >{{ teacher.name }}&nbsp;{{ teacher.levelName }}</span
              >
            </h3>
            <section class="mt10">
              <span class="t-tag-bg">{{ teacher.career }}</span>
            </section>
            <section class="t-infor-txt">
              <p class="mt20">
                {{ teacher.intro }}
                <!-- 北京师范大学法学院副教授、清华大学法学博士。自2004年至今已有9年的司法考试培训经验。长期从事司法考试辅导，深知命题规律，了解解题技巧。内容把握准确，授课重点明确，层次分明，调理清晰，将法条法理与案例有机融合，强调综合，深入浅出。 -->
              </p>
            </section>
            <div class="clear"></div>
          </div>
        </section>
        <div class="clear"></div>
      </div>
      <section class="mt30">
        <div>
          <header class="comm-title all-teacher-title c-course-content">
            <h2 class="fl tac">
              <span class="c-333">主讲课程</span>
            </h2>
            <section class="c-tab-title">
              <a href="javascript: void(0)">&nbsp;</a>
            </section>
          </header>
          <!-- /无数据提示 开始-->
          <section
            class="no-data-wrap"
            v-if="
              teacher.courseList !== undefined &&
              teacher.courseList !== null &&
              teacher.courseList.length == 0
            "
          >
            <em class="icon30 no-data-ico">&nbsp;</em>
            <span class="c-666 fsize14 ml10 vam">讲师还未发布课程呦。。。</span>
          </section>
          <!-- /无数据提示 结束-->
          <article class="comm-course-list">
            <ul class="of">
              <li v-for="course in teacher.courseList" :key="course.id">
                <div class="cc-l-wrap">
                  <section class="course-img">
                    <img :src="course.cover" class="img-responsive" />
                    <div class="cc-mask">
                      <a
                        :href="'/course/' + course.id"
                        title="开始学习"
                        target="_blank"
                        class="comm-btn c-btn-1"
                        >开始学习</a
                      >
                    </div>
                  </section>
                  <h3 class="hLh30 txtOf mt10">
                    <router-link
                      :to="'/teacher/' + course.id"
                      class="course-title fsize18 c-333"
                    >
                      {{ course.title }}
                    </router-link>
                  </h3>
                </div>
              </li>
            </ul>
            <div class="clear"></div>
          </article>
        </div>
      </section>
    </section>
    <!-- /讲师介绍 结束 -->
  </div>
</template>
<script>
import Navbar from "@/components/Navbar";
import { getTeacherById, getTeacherInfoIndexById } from "@/api/teacher/teacher";
export default {
  components: {
    Navbar,
  },
  data() {
    return {
      teacher: {},
    };
  },
  created() {
    // this.init();
    this.queryTeacherInfo();
  },
  methods: {
    queryTeacherInfo() {
      if (this.$route.params && this.$route.params.id) {
        let data = { id: this.$route.params.id };
        getTeacherInfoIndexById(data).then((response) => {
          // console.log("info: ",response.data)
          this.teacher = response.data;
        });
      }
    },
  },
};
</script>