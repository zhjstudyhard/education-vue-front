  ...
  <div class="blog-detail">
    ...
    <div class="mblog">
      ...
      <div class="home-title">
        <h2>{{ blog.title }}</h2>
        <span style="font-size: small;color: blue">创建时间：{{ blog.gmtCreate}}  </span>
        <span style="font-size: small;color: blue">  更新时间：{{ blog.gmtModified}}   </span>
...
export default {
  ...
  methods: {
    ...
    getBlog() {
      ...
      const _this = this
      this.$axios.get('/blog/getBlogById?id=' + blogId).then(res => {
        const blog = res.data.data
        _this.blog.id = blog.id
        _this.blog.title = blog.title
        _this.blog.createTime = blog.gmtCreate
        _this.blog.updateTime = blog.gmtModified
