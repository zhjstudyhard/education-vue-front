...
import { getArticleById,addArticle,getArticlePage,updateArticle,deleteArticleById } from "../../api/article/article";
export default {
  name: "ArticleList",
  data() {
    return {
  ...
  methods: {
    ...
    //获取当前分页的文章
    getData() {
      getArticlePage(this.queryInfo).then((response) => {
    ...
    deleteArticleById(id) {
      ...
      this.$confirm("确认删除当前文章?", "Warning", {
        ...
        .then((response) => {
          deleteArticleById(data).then((response) => {
            this.$message({
              type: "success",
              message: "删除成功!",
            });
