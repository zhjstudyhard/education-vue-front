...
import { uploadFile } from "../../api/article/uploadFile";
export default {
  // name: "BlogEdit",
  data() {
    return {
  ...
  methods: {
    imgAdd(pos, $file) {
      ...
      uploadFile(formdata).then((response) => {
        ...
        this.ruleForm.fileIds = response.data.id;
    ...
    imgDel(pos, $file) {
      ...
      this.ruleForm.fileIds.splice(this.ruleForm.fileIds.indexOf(pos[0].substring(37)),1);
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const _this = this;
          var fileIds = this.ruleForm.fileIds + ''
    ...
    getTypes() {
      ...
      queryDictionaryByType(data).then((response) => {
